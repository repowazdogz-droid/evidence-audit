...
theorem type_to_size_no_panic
  ...
  -- With the RustSized instances, every arm of `type_to_size` computes, so each
  -- goal closes by `rfl` with no size_of hypothesis at all.
  unfold type_to_size
  split <;> exact ⟨_, rfl⟩
...
(self : BinderObjectBytes) :
    ∃ v, self.size = ok v := by
  obtain ⟨t, ht⟩ := hdr_type_no_panic self
  obtain ⟨o, ho⟩ := type_to_size_no_panic t
...
: type_to_size t ⦃ fun _ => True ⦄ :=
  exists_imp_spec (by obtain ⟨v, hv⟩ := type_to_size_no_panic t; exact ⟨v, hv, trivial⟩)

@[local step]
theorem size_spec (self : BinderObjectBytes)
: self.size ⦃ fun _ => True ⦄ :=
  exists_imp_spec (by obtain ⟨v, hv⟩ := size_no_panic self; exact ⟨v, hv, trivial⟩)
...
/- =====================================================================
   ...
   `core.mem.size_of` is now a total definition rather than an opaque axiom
   (see the note at its declaration in SpikeBinderRemodel.lean and PLAN.md),
   so `core.mem.size_of_total` discharges the `hsz` hypothesis outright and
   these theorems hold with no assumptions at all.
   ===================================================================== -/

theorem type_to_size_no_panic_uncond (type_ : Std.U32) :
    ∃ r, type_to_size type_ = ok r :=
  type_to_size_no_panic type_

theorem size_no_panic_uncond (self : BinderObjectBytes) :
    ∃ v, self.size = ok v :=
  size_no_panic self

/-- The maintainer's target property, unconditionally: for every byte input,
    `parse_one` returns `ok` — it never panics. -/
theorem parse_one_no_panic_uncond (bytes : Slice Std.U8) :
    ∃ r, parse_one bytes = ok r :=
  parse_one_no_panic bytes

/- Axiom audit. Each of these must list only the standard Lean axioms
   (`propext`, `Classical.choice`, `Quot.sound`) — in particular no
   `spike_binder_remodel.core.mem.size_of` and no `sorryAx`. -/
#print axioms type_to_size_no_panic_uncond
#print axioms size_no_panic_uncond
#print axioms parse_one_no_panic_uncond
#print axioms parse_one_no_panic

/- =====================================================================
   Layout fidelity gate.

   These pin the per-type sizes the extraction relies on. They are proved by
   `rfl` against the `RustSized` instances, and they FAIL to compile if an
   instance is edited to a wrong value, so they are a regression guard on the
   layout model rather than documentation. The values match the Rust
   `const _: () = { assert!(size_of::<..>() == ..) }` checks in
   `remodel/src/lib.rs`, and the kernel UAPI header they came from.
   ===================================================================== -/

theorem size_of_flat_binder_object :
    core.mem.size_of uapi.flat_binder_object = ok 24#usize := rfl
theorem size_of_binder_fd_object :
    core.mem.size_of uapi.binder_fd_object = ok 24#usize := rfl
theorem size_of_binder_buffer_object :
    core.mem.size_of uapi.binder_buffer_object = ok 40#usize := rfl
theorem size_of_binder_fd_array_object :
    core.mem.size_of uapi.binder_fd_array_object = ok 32#usize := rfl
theorem size_of_usize :
    core.mem.size_of Std.Usize = ok 8#usize := rfl

/-- `type_to_size` now computes the true object size for a real tag. -/
theorem type_to_size_ptr :
    type_to_size BINDER_TYPE_PTR = ok (some 40#usize) := by
  unfold type_to_size BINDER_TYPE_PTR; rfl

#print axioms size_of_binder_buffer_object
#print axioms type_to_size_ptr
